...
> **Why the environment is so bare:** your interactive prompt only has a fat `$PATH`, aliases, and env vars because logging in *sourced* `/etc/profile` and `~/.bashrc`. A cron job never logs in — cron just forks and runs the line — so none of that setup happened. That's the entire reason "works in my shell, fails in cron": spell out absolute paths (`/usr/bin/python3`) and any `export`s the job needs, because nothing set them up for you.
