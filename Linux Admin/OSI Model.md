...
> **Encapsulation = nested envelopes.** Your HTTP request (L7) is sealed inside a TCP envelope (L4 — ports), which goes inside an IP envelope (L3 — IP addresses), which goes inside an Ethernet frame (L2 — MACs) before a single bit hits the wire; the receiver opens them outermost-first (1→7). The detail that makes routing click: at **every router hop the L2 envelope is torn off and rewritten** with new source/dest MACs for the next link, while the **L3 (IP) envelope rides unchanged** the whole way. That's *why* MAC is "local link only" and IP is "end-to-end routable" — you're literally watching the envelopes get relabeled hop by hop.
