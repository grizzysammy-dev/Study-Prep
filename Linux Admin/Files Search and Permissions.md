...
Finding files, copying/imaging them, searching inside them, and controlling who can touch them — plus bundling and compressing. Permissions are the part the exam loves to test with a `drwxr-xr-x` → number conversion.
...
## Concept
On Linux **everything is a file**, and every file has an **owner**, a **group**, and a 3×3 grid of read/write/execute bits (owner / group / other). Two search tools: `find` walks the disk **live** (accurate, slower); `locate` queries a prebuilt **index** (instant, possibly stale). `grep` searches file *contents*. `tar` *bundles*; `gzip`/`xz`/`bzip2` *compress* — combine them for `.tar.gz` etc.
...
```
→ `-exec cmd {} \;` runs once **per file**; `-exec cmd {} +` **batches** many files into one call. Combine tests: `find / -type f -size +50M -mtime -7` = large files changed this week.
> **Why `-perm -4000` is a priv-esc hunt:** a file with the **setuid** bit runs as its *owner*, not as whoever launched it — that's how an unprivileged user running `passwd` can edit root-owned `/etc/shadow`. So "find SUID binaries" really means "show me every program that lends out its owner's privileges." On offense that's a hunt for a root-owned one with a known escape (GTFOBins, in the refs); from the defender's chair it's an integrity check — an unexpected SUID-root bit is how a foothold quietly becomes root.
...
```
→ Core flags to remember: `-i -r -n -v -E -o -c`. The everyday combo for logs is `grep -rin "phrase" /var/log`.
...
```
→ `find` printed the first three matching config files; the `grep` returned **nothing** — `/etc/rsyslog.conf` has no line containing "listen" (a no-match is silent, not an error). Honest reminder: an empty `grep` result means "not found here," so widen the path (`grep -rin listen /etc/rsyslog.d/`) before concluding.

## locate / updatedb — instant name search from an index
```bash
...
```
> **Gotcha:** `locate` only knows what existed at the **last `updatedb`** — a file created 5 minutes ago won't show until you re-run it. Use `find` for real-time, `locate` for speed.
...
```
→ For backups/migrations use `cp -a` so ownership and timestamps survive.

## dd — low-level copy / image / backup (DANGEROUS)
`dd` copies raw blocks — image a whole disk, write an ISO to USB, back up a partition bit-for-bit.
```bash
...
```
→ `if=` **in**put, `of=` **out**put, `bs=` block size (bigger = faster), `status=progress` = live progress.
> ☢️ **DANGER — "disk destroyer":** `dd` writes exactly where `of=` points with **no confirmation**. Swapping `if`/`of` or a wrong `/dev/sdX` **instantly and irrecoverably overwrites** a disk. **Triple-check device names with `lsblk` first.**
> 🧪 **Run this on your lab** — verified against current docs, confirm on your box. Needs real disks/USB the sandbox doesn't have.

## vim — very brief
Enough to edit a config and get out. Full note: [Vim](../Vim/Vim.md).
```
vim file.txt      # open
...
```
> **#1 beginner trap:** you can't type until `i`, and can't quit until `Esc` then `:q`. `:q!` is the emergency exit.
...
```
- **r**=read (4), **w**=write (2), **x**=execute (1). On a **directory**, `x` = "may enter/cd in," `r` = "may list names."
- Add per triad: `rwx`=7, `r-x`=5, `rw-`=6, `r--`=4. So **`drwxr-xr-x`** = directory, owner **7**, group **5**, other **5** = **755**.

> **How the kernel actually checks (it does *not* add the triads up):** it asks in order — *are you the file's owner?* If yes, only the **owner** bits apply and it stops looking. *No — are you in its group?* Then only the **group** bits apply. *Neither?* You're **other**. Exactly one triad decides, first-match — which is why a file can be `r--` for its group yet `---` for its owner and the owner is genuinely locked out despite "outranking" the group.

**What `777` means:** `rwxrwxrwx` = **everyone** (owner, group, other) can read, write, AND execute.
> **Security red flag:** 777 lets *any user on the system* modify or run the file — almost never correct. Sensible defaults: files **644** (`rw-r--r--`), directories **755**, private key/secret **600** (`rw-------`), scripts **755** or **700**.
...
```
→ Confirms the mapping: `754` on the file renders as `-rwxr-xr--`, `755` on the dir as `drwxr-xr-x`, and `777` opens it wide to `-rwxrwxrwx`. Read the string right-to-left in triads to reverse it back to the number.
...
```
> **Gotcha:** `chmod -R 777` on a whole tree is the classic "just make it work" mistake — it opens *and* makes every data file executable. Prefer the `find … -type d/-type f` split above. Only **root** (or the owner, for chmod) can change ownership.

## Compression & Archiving
**Key concept:** **`tar`** *bundles* many files into one archive (`.tar`) but doesn't compress; **`gzip`/`xz`/`bzip2`** are *compressors* that shrink a stream. Combine them → `.tar.gz`, `.tar.xz`, `.tar.bz2`.

**`tar` — the workhorse (learn `czvf` / `xzvf`):**
```bash
...
```
→ Memory hook: **c**reate / e**x**tract / lis**t**; **f** = "filename next" (always last); `v` = verbose. Modern `tar xf file.tar.*` often auto-detects the compressor.
...
```
> **Quick chooser:** `.zip` for sharing with **Windows**; `.tar.gz` = universal Linux default; `.tar.xz`/`.7z` = **smallest**; `.tar.bz2` = older middle-ground.
...
-rwxrwxrwx 1 root root    0 Aug 17 04:02 file1
```
→ Same 10-byte `data.txt` bundled four ways. On a payload this tiny the container overhead dominates (zip is biggest at 176 B), so ratios only mean something on real data — the point here is that each format produced its file and round-tripped.
...
- An empty `grep` result is a **no-match**, not a failure — broaden the path/pattern before concluding.
