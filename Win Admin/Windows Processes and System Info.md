...
jqr: "Processes: tasklist/taskkill and Get-Process/Stop-Process, finding where a process runs from (Get-CimInstance Win32_Process ExecutablePath, since wmic was removed in 2026), systeminfo/Get-ComputerInfo, mapping a network drive, and GUI file manipulation"
---

# Windows Processes and System Info

Listing processes, **killing** them, finding **where a process runs from on disk** (the defensive money question), pulling **system info**, mapping a **network drive**, and the File Explorer GUI tricks. This is the Windows counterpart to [Processes and systemd](../Linux%20Admin/Processes%20and%20systemd.md).
...
```
> **2026:** `wmic.exe` is **removed**, so I use `Get-CimInstance` for `ExecutablePath`/`CommandLine`. WMI itself still works, only the old wrapper is gone.

## What matters about a process
Every running program is a **process** with three things I care about as a defender:
- a **PID** (process id), the unique number I kill by,
- an **image name** (`notepad.exe`), the on-disk file it was launched from, and
- an **executable path**, meaning *where* that file actually lives on disk.

That last one is the tell. Legit Windows binaries live in `C:\Windows\System32`, so the same-named process running out of `C:\Users\...\AppData\Temp` is how masquerading malware hides. `tasklist`/`Get-Process` show the first two easily, but the **path and the launch command line** need `Get-CimInstance Win32_Process` (the 2026 replacement for the retired `wmic`). Pair that with a quick **`systeminfo`** to fingerprint the host and I can triage an unfamiliar machine in about two commands.

> **Why the path beats the name:** a process name is just the filename of the image it loaded, which is trivially faked since anyone can name a binary `svchost.exe`. The **full path** (and its digital signature) is what's hard to fake: the real `svchost.exe` is a signed file in `C:\Windows\System32`, so a `svchost.exe` running from a user's Temp folder is that name wearing a disguise. Under the hood a process is an isolated block of memory plus a **security token** (the account and privileges it runs as) and one or more threads, and the PID is just the kernel's handle to that whole bundle.
...
```
`tasklist /svc` is great for triage, since it tells me which service is hiding inside a shared `svchost.exe`. (Windows packs many services into a handful of shared `svchost.exe` host processes to save resources, so one `svchost` PID can be running a dozen services at once, and `/svc` is how I see which ones.)

## Where a process runs from (executable path)
This is the defensively important one: a `svchost.exe` running from `C:\Users\...\Temp` instead of `C:\Windows\System32` is a red flag.
```cmd
...
```
(do this on the Windows VM, but it matches the docs). `Get-CimInstance Win32_Process` with `ExecutablePath` **and** `CommandLine` is my answer to "where did this run from, and how was it launched."

> **Gotchas:** `wmic` no longer exists on 2026 Windows 11, so I write `Get-CimInstance` as the real answer (and only add the `wmic` line if an older exam explicitly accepts it). `Get-Process ... Path` can show **blank** for protected or system processes unless the shell is **elevated**.

> **What WMI/CIM actually is:** picture a built-in **database describing the whole machine** (processes, services, disks, patches, NICs) that I query like a data source. `wmic` was the old command-line wrapper over it (now removed), and `Get-CimInstance` is the modern client to the *same* repository. `Win32_Process` is basically "the processes table," so asking it for `ExecutablePath`/`CommandLine` is a query, which is why it surfaces fields `tasklist` never shows.
...
```
`/F` is force, `/IM` is image name, `/PID` is process id, `/T` is the whole tree. Killing by `/IM` hits every copy, so I use a PID when I want to be surgical.
...
```
GUI equivalent is **`msinfo32`**. `systeminfo` is my fast "what OS/patch level/domain is this" check. **Gotcha:** `Get-ComputerInfo` is thorough but takes a few seconds, which is normal, not a hang.
...
```
> **Gotcha:** plain `New-PSDrive` (no `-Persist`) only exists inside that PowerShell session, so it won't show in Explorer or survive a reboot. I use **`-Persist`** for a real mapped drive. (Why: a mapped drive is a saved SMB client session, and `-Persist` writes it to the registry so Windows rebuilds it at logon, whereas a session-only drive lives in RAM and dies with the shell. Same in-memory-vs-saved split as `route` vs `route -p`.)
...
- **Share a directory:** right-click folder → **Properties** → **Sharing** tab → **Advanced Sharing...** → tick **Share this folder** → set **Share name** (`data`) → **Permissions**. Remember NTFS (Security) perms also apply, and effective access is the **more restrictive** of share vs NTFS.

> **Win11 context-menu gotcha:** Windows 11 shows a **compact** right-click menu first. If the option I want isn't there, I click **Show more options** (or Shift+F10) for the full classic menu.

## Gotchas
- **`wmic` is gone in 2026**, so `Get-CimInstance Win32_Process` is the executable-path/command-line answer.
- **Blank process Path?** Elevate the shell, because protected and system process paths are hidden otherwise.
...
- **Share vs NTFS:** effective access is the more restrictive layer, in both CLI and GUI.
