...
```
> Prefer **adding a rule** over disabling the firewall. Disabling is a troubleshooting step, not a fix — turning it *off* drops **all** filtering (every port open to anyone who can route to you), while a rule opens exactly one port to exactly the hosts you name. On the blue side a disabled firewall is a finding; a scoped allow rule is hygiene.

## Concept
The firewall itself is a **stateful packet filter** in the kernel's Windows Filtering Platform: every packet is checked against rules that match on direction, protocol, port, and address. *Stateful* means it remembers connections **you** started, so replies to your own outbound traffic are let back in without a rule. That's the key asymmetry — Windows **default-allows outbound** and **default-blocks unsolicited inbound** — and it's why the rules you actually write are almost always *inbound allow* rules poking a hole for a service you're hosting.
...
Rules are per-profile, so "it's blocked on Public but works on Private" is normal. When troubleshooting "can't reach the service," check *which profile the adapter is on* and whether the rule covers that profile. This is the Windows counterpart to [iptables](../IP%20Tables%20CentOS/iptables.md).

> **Why profiles exist:** it's one laptop but many networks with different trust levels. Windows auto-tags each connection with exactly one profile — *Domain* if it can reach your org's domain controller, *Private* if you marked it trusted, else *Public* — and scopes rules to profiles so your relaxed home rules don't follow you onto café Wi-Fi. `Get-NetConnectionProfile` tells you which one is live right now.
...
- **ICMP (ping) is not a port** — allow it by protocol (`protocol=icmpv4` / `-Protocol ICMPv4`), not a `localport`. *(Ports are a TCP/UDP concept; ICMP is its own IP protocol with no ports at all, so there's nothing to put in `localport`.)*
