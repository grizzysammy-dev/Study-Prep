...
Every conversation on a network is a stream of packets: headers (who/where/which port/flags) plus a payload (the actual data). Normally your NIC is polite — it checks each arriving frame's destination MAC and silently drops anything not addressed to it. **Promiscuous mode** switches off that filter so the card keeps *everything* it hears, and tcpdump copies those packets up and shows you the fields. Two caveats that trip people up: you only see traffic **on-path** — a switch already forwards each frame only toward the port that owns the destination MAC, so on a switched LAN you mostly see just your own box unless you're at a SPAN port/tap — and the payload is only readable if the *application* didn't encrypt it, because tcpdump adds no decryption. That second point is the entire cleartext demo below.
...
Pick the interface deliberately — capturing on the wrong NIC/VLAN is the #1 reason you "see nothing." See [Interfaces IPs and Routing](../Linux%20Admin/Interfaces%20IPs%20and%20Routing.md) for finding the right one.

## BPF filters — choose what to capture

Filters go at the **end**, after options.

> **Why filtering is fast (and why you filter at capture time):** a BPF filter isn't `grep` on the output — it's a tiny program compiled and pushed *into the kernel*, which tests and drops non-matching packets before they're ever copied up to tcpdump. So on a busy link you scope the capture instead of grabbing everything and sifting later: the kernel throws the noise away for you, cheaply, at the source.
...
Before blaming the network, confirm the service is actually bound and watch who's connected with `ss -tulpn` (`-t` TCP, `-u` UDP, `-l` listening, `-p` process, `-n` numeric). Deeper coverage in [Processes and systemd](../Linux%20Admin/Processes%20and%20systemd.md).
