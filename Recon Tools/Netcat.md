...
> **Why a banner IDs the service:** many protocols have the *server* greet you first — announcing its version so the client knows what it's talking to before either side does real work. netcat just opens the socket and prints that greeting; no magic, you're reading the service introduce itself. HTTP is client-first, so you send the `HEAD` request and read its reply instead.
...
> **Why reverse wins, and the blue-side tell:** firewalls are usually default-deny *inbound* but default-allow *outbound* (users need to browse out), so a bind shell's new listening port often gets blocked while a reverse connection sails straight through. The flip side for a defender: a reverse shell is an *outbound* connection from a box that should only ever *receive* them — a web server suddenly dialing out to a random high port is the anomaly. Watch egress, not just ingress.
...
> **Why the FIFO works:** with no `-e`, you wire the plumbing by hand. `mkfifo` makes a *named pipe* — a one-way tube on disk. `nc ... < /tmp/f` feeds whatever's in the pipe into netcat's input, `| /bin/bash` runs whatever the attacker types as commands, and `> /tmp/f` loops bash's output back into the pipe for netcat to send out. The FIFO is just the loop-back wire reconnecting bash's mouth to netcat's ear — hand-building the socket-to-shell bridge that `-e` would give you for free.
