...
This is the vulnerability zoo I'm expected to name and explain. I split it the way it actually divides: **memory-corruption** (native-code) bugs on one side, **web / application-logic** bugs on the other, each mapped to its **CWE** and its **OWASP Top 10** slot. The JQR calls out **use-after-free, heap overflow, and SQL injection** by name, so those are anchored inside the full set below.

## Quick recall
- A **vulnerability** is the flaw. An **exploit** is the code/technique that abuses it. (I go deeper on that split in [Zero-Day Exploits](Zero-Day%20Exploits.md).)
- The three the JQR names: **Use-after-free** (CWE-416), **Heap overflow** (CWE-122), **SQL injection** (CWE-89).
- Fix mantras I want reflexive: SQLi → **parameterized queries / prepared statements**; XSS → context-aware output encoding; XXE → **disable DTD/external entities**; CSRF → **anti-CSRF token + SameSite cookies**; deserialization → don't deserialize untrusted data.
- Memory bug to code execution, the pipeline: corrupt a value the CPU trusts (return address / function pointer / vtable) → **ROP** to bypass **DEP/NX** → run **shellcode**. Broken by **ASLR, stack canaries, CFG/CET**.
- OWASP: I lead with **2025** (Injection is now **A05**, SSRF is no longer standalone) but I stay ready for a **2021** answer key (Injection was A03).

## How I think about it
A **vulnerability** is a flaw (in code, design, or config) that can be abused to break confidentiality, integrity, or availability. Two broad families matter for this JQR:

- **Memory-corruption / native-code bugs** live in C/C++-style software with manual memory management. The picture I use: memory is a row of labeled mailboxes, and a buffer overflow stuffs so much into one box that it spills into the next. If that neighbor happens to hold the "where do I go next?" address the CPU trusts, I've just steered the program. These let an attacker overwrite memory the program never meant to expose and, worst case, hijack execution. This is the family behind most browser and kernel "0-click" exploits.
- **Web / application-logic bugs** live in how an app handles untrusted input and trust boundaries. They rarely need memory tricks. They abuse the app doing exactly what it was (badly) told to do. I'm not picking the lock here, I'm tricking the doorman who trusts whatever I tell him.
...
### How a memory bug turns into code execution (the *idea*, not a weapon)
Here's the pipeline conceptually. A bug lets me overwrite a value the CPU trusts for control flow, a **return address**, a **function pointer**, or a **C++ vtable** pointer, and point it at code I influence. Modern OSes mark data pages non-executable (**DEP/NX**), so I can't just drop shellcode and jump to it. So first I use **ROP (Return-Oriented Programming)**, chaining tiny snippets ("gadgets") of the program's own existing code, to disable protections or allocate executable memory, then run **shellcode** (a small position-independent payload). The defences that break this chain: **ASLR** (randomizes addresses so gadgets can't be located), **stack canaries** (detect stack overwrites), **CFG/CET** (control-flow integrity), and **DEP/NX**. The exam wants this pipeline described conceptually, not a working exploit.
...
## Real-world anchor
**CISA AA21-209A** ("Top Routinely Exploited Vulnerabilities," Jul 2021, joint US/AU/UK) showed attackers overwhelmingly reuse **known, unpatched CVEs** rather than burn fresh zero-days. That's the practical argument for patch management over exotic-bug paranoia.

## What trips me up
- The three the JQR calls out by name: **use-after-free, heap overflow, SQL injection.** I want their one-line explanations ready.
