...
The device/control zoo, plus the detection techniques behind them. I already run half of these from the blue chair, so this note is really about nailing the crisp definitions and the one offensive term, **PSP**, that the JQR wants me to state plainly.
...
## Two questions that organize all of it
Two questions organize this whole topic for me: **where does the control sit** (network boundary, endpoint host, or across the whole estate) and **how does it decide what's bad** (matching known signatures vs modelling normal and flagging the deviations). Once I've got those two axes, every device below slots into place.

The picture I use is building security: the **firewall** is the gate guard working a guest list; the **IDS** is the CCTV operator who watches and phones it in; the **IPS** is the guard standing in the doorway who can actually stop you; **EDR** is a bodyguard assigned to each person inside; the **WAF** screens web visitors specifically; the **honeypot** is a fake vault wired to an alarm, and since nobody has a legitimate reason to touch it, *any* touch is a real signal. I already run most of these from the blue chair. The one genuinely new word here is the offensive one, **PSP**, which is the same endpoint stack named from the attacker's side as the thing they have to evade.

## The devices

- **Firewall**: enforces an allow/deny policy at network boundaries.
  - *Stateful firewall*: tracks connection state (the TCP session table) and auto-allows return traffic for established flows. A step beyond simple stateless packet filtering.
...
- **EDR / XDR**: **EDR (Endpoint Detection & Response)** is a host agent recording process/file/network telemetry for detection, threat-hunting, and response (isolate/kill/roll-back). **XDR (Extended Detection & Response)** correlates across endpoint **+ network + email + cloud + identity** for unified detection/response.
- **PSP (Personal Security Product).** State it plainly: **PSP = the endpoint's host AV / endpoint-protection product(s).** In an **offensive / DoD red-team context**, "PSP" is the operator's term for the host-based protection they have to be *aware of and evade*: **AV / anti-malware / EDR / host firewall / HIPS** on the target endpoint. Defensively it's just my endpoint stack, offensively it's the thing that flags tooling, which is where tradecraft like **AV evasion** and **AMSI bypass** comes in.
- **Proxy**: an intermediary for requests. A *forward proxy* controls/monitors **outbound** user traffic (URL filtering, caching, egress control). A *reverse proxy* fronts servers (load-balancing, TLS termination, hides origin).
- **WAF (Web Application Firewall).** An **L7 filter for HTTP(S)** aimed at **web attacks** (SQLi/XSS/traversal) via signatures + anomaly rules (e.g. the OWASP Core Rule Set). Mitigates OWASP Top 10 classes, see [Common Software Vulnerabilities](Common%20Software%20Vulnerabilities.md).
...
- **Honeypot**: a **decoy** system/service with no legitimate use, so *any* interaction is inherently suspicious, which gives high-fidelity detection plus attacker intel. (**Honeytokens** are the data-object equivalent, a fake credential/file whose use trips an alarm.)
...
- **Anomaly / heuristic / behavioral**: models "normal" and flags deviations (unusual beaconing, odd process behavior). Catches **unknown** threats but generates **more false positives**; underpins UEBA / ML detection.
