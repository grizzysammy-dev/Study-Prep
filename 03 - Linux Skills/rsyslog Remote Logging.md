...
Open the firewall for it (ties to [iptables](iptables.md)):
```bash
...
sudo tcpdump -ni any 'port 514'          # confirm packets on the wire — see [Tcpdump](../02%20-%20Recon%20and%20Network%20Tools/Tcpdump.md)
```
...
- **Server not receiving? Check in this order:** (1) listener up — `ss -tulnp | grep 514`; (2) firewall allows 514 — [iptables](iptables.md); (3) on CentOS, SELinux may block the port — `sudo semanage port -a -t syslogd_port_t -p tcp 514`.
