...
SSH encrypts the whole session, so `ssh`, [SCP](SCP.md), and every tunnel ride one authenticated, encrypted channel — unlike cleartext tools like [Netcat](../Recon%20Tools/Netcat.md). Picture the connection as a sealed tube already strung between you and the server. A forward stuffs a *second* stream of traffic into that tube; at the far end **sshd acts as your proxy** — it opens a fresh, ordinary TCP connection to the real destination *on your behalf*, so the destination sees the traffic arriving **from the SSH server**, not from you. That one fact explains everything below.

Two ideas cover the rest:

- **Port forwarding = "poke a hole through the SSH pipe."** The only thing you must get right is **which end opens the listening socket**. Analogy: the tube is already strung — a forward just decides *whose wall gets the mail slot*. `-L` puts the slot on **your** wall (you drop traffic in, it emerges at the server's end); `-R` puts it on the **server's** wall (traffic dropped in there travels back to you). Delivery always happens at the *opposite* end from the slot.
...
> **Why you'd reach for `-R`, and the blue-side view:** it's the escape hatch for a host you can't dial *into* — the box reaches *out* and hands one of its local services back to you over that outbound session. That's the classic pivot/exfil primitive, same shape as a reverse shell: on the defender's side the tell is an *outbound* SSH connection from a server that has no business initiating one.
...
> **Why `-D` is the pivot workhorse:** SOCKS is just a tiny "dial *this* (host, port) for me, whatever it is" protocol. Where `-L` is one fixed pipe to one service, `-D` turns the bastion into a general-purpose exit node onto the internal network — map the subnet, reach any host/port, all through the single SSH session. That's why it pairs with `proxychains`: point un-proxy-aware tools at the one SOCKS port and they all ride the tunnel.
