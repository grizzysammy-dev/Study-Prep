...
Under the hood `scp` opens a normal SSH session and runs a file-transfer program on the far end — that's the *entire* trick. Nothing about the security is new: it's the same SSH channel you already trust, carrying the bytes of a file instead of the keystrokes of a shell. So every SSH nicety comes along for free.
