...
> **The mental model:** a shell script is nothing more than the exact commands you'd type at the prompt, saved in a file and run top-to-bottom for you. Bash *is* a real programming language — but its "standard library" is every command already on the box (`ls`, `grep`, `nmap`, …), and its whole job is to glue them together. Think recipe, not app: the shell reads one line, does literally what it says, moves to the next. Once you see it that way, "scripting" is just "typing, but saved and repeatable."
...
> **Why the shebang matters:** when you run `./script.sh`, the kernel peeks at the first two bytes of the file. If they're `#!`, it reads the rest of that line and hands your file to *that* interpreter. So the shebang is how a plain text file gets to declare "I'm a bash program" (or python, or perl) — without it, the OS is left guessing.
...
> **Why the `./`?** The shell won't run a script from the current folder just because you typed its name — otherwise a malicious file named `ls` dropped in a directory could hijack you. `./hello.sh` says "yes, run *this* file, right here, on purpose."
...
> **Why no spaces around `=`?** The shell decides what a line *is* by its first word. `x=1` has no spaces, so it reads as an assignment; `x = 1` looks like "run the command `x` with arguments `=` and `1`." Same reason quoting matters — the shell splits unquoted values on spaces *before* the command ever sees them, so `"$greeting"` stays one argument instead of becoming two. `$(...)` is the quiet workhorse: it runs a command and drops its output straight into the line, which is how you wire tools into each other.
...
```
→ "Do this to every host / port / line" is most of what recon and log-parsing actually are — the loop is the engine. That last one, `until ping … do sleep 1`, is a whole wait-for-the-box-to-come-up watcher in a single line.
...
> **Why exit codes are the backbone:** `if`, `&&`, and `||` don't test "true/false" — they test the *exit code* of the last command. `cmd && echo ok` means "run echo only if cmd returned 0"; `||` fires only on failure. That one convention — 0 means success — is what lets you wire commands into logic with no glue code at all.
...
```
> **Why the number in `2>`:** every process is born with three streams — stdin (0), stdout (1), stderr (2). Normal output flows out 1, errors out 2, deliberately kept apart so you can throw away the noise and keep the signal. A pipe `|` just wires one program's stdout straight into the next one's stdin — an assembly line where each tool does one job and passes the work along.
...
```
> **Why `set -euo pipefail`:** by default bash shrugs off errors and marches on — a script whose `cd` failed will cheerfully run the next line in the wrong directory. This line makes it stop on the first error, treat an unset variable as the mistake it usually is, and refuse to hide a failure buried in a pipe. *Blue-team aside:* those same cron entries are prime persistence real estate — a stray `@reboot` line in someone's crontab is exactly what you'd hunt for as an attacker's foothold, so it pays to read them from both chairs.
