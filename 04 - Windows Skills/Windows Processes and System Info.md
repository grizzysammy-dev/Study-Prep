...
Listing processes, **killing** them, finding **where a process runs from on disk** (the defensive money question), pulling **system info**, mapping a **network drive**, and the File Explorer GUI tricks. Windows counterpart to [Processes and systemd](../03%20-%20Linux%20Skills/Processes%20and%20systemd.md).
...
```
→ `tasklist /svc` is great for triage — it tells you which service is hiding inside a shared `svchost.exe`.
...
```
> **Gotcha:** plain `New-PSDrive` (no `-Persist`) only exists inside that PowerShell session — it won't show in Explorer or survive a reboot. Use **`-Persist`** for a real mapped drive.
