...
`icacls` reads and edits the **NTFS ACL** — who can do what to a file or folder. It's the Windows analogue of `chmod`/`chown` + ACLs from [Files Search and Permissions](../03%20-%20Linux%20Skills/Files%20Search%20and%20Permissions.md), but expressed as per-user entries with inheritance flags. Editing permissions on files you don't own needs **Administrator**.
