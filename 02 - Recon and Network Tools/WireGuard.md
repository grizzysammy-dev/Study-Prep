...
- On the wire it's **UDP-only and silent** — it never replies to non-peers, so it's stealthier than a TLS VPN. For a blue-teamer the *presence* of an unexpected `wg0` interface or UDP peer is itself the signal.
...
```
→ Installs the `wg` / `wg-quick` userspace tools; the kernel module ships with modern kernels. On kernels older than 5.6 you'd need `wireguard-dkms` — not needed on 2026 distros, nothing to compile.
...
- **Lazy handshake:** `wg show` may list a peer with *no* handshake until the first packet — `ping` the peer, then re-check.
- Persist across reboot: `sudo systemctl enable wg-quick@wg0`.
- No handshake ever? Firewall is dropping **UDP/51820** (see [iptables](../03%20-%20Linux%20Skills/iptables.md)), the `Endpoint` is wrong, or the public keys are swapped. If one side is behind NAT, add `PersistentKeepalive = 25` to its `[Peer]` block so the mapping stays open.
- To route a whole **subnet** through a peer, widen its `AllowedIPs` (e.g. `172.16.5.0/24`) and enable `net.ipv4.ip_forward=1` — see [Interfaces IPs and Routing](../03%20-%20Linux%20Skills/Interfaces%20IPs%20and%20Routing.md).
