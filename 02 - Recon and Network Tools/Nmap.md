...
The network mapper: it sends crafted packets, reads the replies, and answers four questions — *is the host up, which ports are open, what service/version is behind each, what OS.* This is almost always your first move against a target, and its XML output feeds straight into [Metasploit Workflow](../05%20-%20Metasploit%20and%20Exploitation/Metasploit%20Workflow.md).
...
From the blue side you *watch* these scans land on your sensors; here's what the attacker is actually doing. Nmap probes a port and reads the TCP reply: a `SYN` that gets `SYN/ACK` = **open**; `SYN` → `RST` = **closed** (host up, nothing listening); `SYN` → silence = **filtered** (a firewall dropped it). Version detection then talks to each open port and matches the banner against a fingerprint database. OS detection guesses from quirks in the target's TCP/IP stack. That's the whole game — everything below is just flags to control it.
...
- [Netcat](Netcat.md)
- [Tcpdump](Tcpdump.md)
- [Enumeration Tools](../05%20-%20Metasploit%20and%20Exploitation/Enumeration%20Tools.md)
