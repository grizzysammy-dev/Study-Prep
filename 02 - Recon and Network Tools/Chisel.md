...
- **Tunnelling = encapsulation:** wrap protocol X inside protocol Y so X can cross a network that would otherwise block it. Chisel's outer wrapper is **HTTP/WebSocket** — the one thing perimeter proxies are most likely to permit. Compare the wrappers: [OpenVPN](OpenVPN.md) = IP-in-TLS, [WireGuard](WireGuard.md) = IP-in-UDP, SSH forwards = TCP-in-SSH, Chisel = TCP/UDP-in-HTTP.
- It's the tool of choice for **pivoting** through tight egress filtering, including **reverse** tunnels (connection dialed client -> server, traffic flowing the other way) to reach a host with no inbound access — see [Pivoting and Tunneling](../06%20-%20Knowledge%20Requirements/Pivoting%20and%20Tunneling.md).
