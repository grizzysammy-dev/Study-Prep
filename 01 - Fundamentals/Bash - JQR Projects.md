...
```
**Concepts shown:** `read -p` input, variables, quoting, a here-block of `echo`s. It builds the exact strings so you can copy the one you need. See [Nmap](../02%20-%20Recon%20and%20Network%20Tools/Nmap.md) for what each flag does.
...
```
**Concepts shown:** `set -e` (stop on error), `||` fallback, `&&` chaining, backgrounding with `&`, scripting [tmux](tmux%20and%20Terminator.md). Swap the tmux block for `terminator &` if you chose Terminator.
...
- Keep both scripts executable (`chmod +x`) and committed so instructors can see them ([Git and GitHub](Git%20and%20GitHub.md)).
