...
- **Memory-corruption / native-code bugs** live in C/C++-style software with manual memory management. They let an attacker overwrite memory the program didn't intend to expose, and — in the worst case — hijack execution. This is the family behind most browser/kernel "0-click" exploits.
- **Web / application-logic bugs** live in how an app handles untrusted input and trust boundaries. They rarely need memory tricks; they abuse the app doing exactly what it was (badly) told to do.
