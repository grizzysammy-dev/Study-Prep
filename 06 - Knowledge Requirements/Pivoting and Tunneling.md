...
**Pivoting** turns one foothold into access to a segmented network. Say you land on a DMZ web server that can talk to an internal VLAN you can't reach from your box — you route your traffic *through* that server to reach the internal hosts. It's the mechanism behind lateral movement, and it's exactly the "Through" arc the Unified Kill Chain adds (see [Cyber Kill Chain](Cyber%20Kill%20Chain.md)). Pivoting is about **reachability**.

**Tunneling** wraps traffic of one protocol inside another so it survives a hostile path. Wrapping arbitrary TCP inside SSH, or inside HTTPS, lets it pass a firewall/proxy/IDS that would otherwise block or read it. Tunneling gives you **transport, evasion, and confidentiality** — the encapsulating protocol both hides and protects the inner traffic. (For why the *encapsulation* matters: raw protocols like netcat travel in cleartext; a tunnel is what makes the same bytes private and firewall-passable — contrast the cleartext capture in [Netcat](../02%20-%20Recon%20and%20Network%20Tools/Netcat.md).)
...
|---|---|---|
| **SSH** | Port forwarding: local (`-L`), remote (`-R`), dynamic SOCKS (`-D`); jump hosts via `ProxyJump`. | You have SSH creds/keys on a host — quick, encrypted, ubiquitous. **The default pivot.** → [SSH - Tunneling and Jump Hosts](../02%20-%20Recon%20and%20Network%20Tools/SSH%20-%20Tunneling%20and%20Jump%20Hosts.md) |
| **iptables** | Linux netfilter — NAT / `REDIRECT` / `MASQUERADE`, forwarding rules. | You're on a compromised Linux router/gateway and need **OS-level packet forwarding/relay**, not an app tunnel. → [iptables](../03%20-%20Linux%20Skills/iptables.md) |
| **OpenVPN** | Full TLS-based VPN tunnel (routed or bridged). | You need a **stable, full-subnet** virtual link — many hosts/protocols — and can tolerate its heavier footprint. → [OpenVPN](../02%20-%20Recon%20and%20Network%20Tools/OpenVPN.md) |
| **WireGuard** | Modern, lean UDP VPN (small codebase, strong crypto). | You want a **low-latency, low-overhead persistent** VPN pivot; simpler and faster than OpenVPN. → [WireGuard](../02%20-%20Recon%20and%20Network%20Tools/WireGuard.md) |
| **Chisel** | TCP/UDP tunnel over **HTTP/WebSocket**, single Go binary, built-in SOCKS5. | Egress is **locked to web/HTTP(S)** — you need a portable client/server to punch through a restrictive firewall. → [Chisel](../02%20-%20Recon%20and%20Network%20Tools/Chisel.md) |
...
proxyjump bastion
```
→ `ssh -G` prints the *effective* parsed config: connecting to `internal-web` will first transit `bastion` (`10.10.10.5`). That `ProxyJump` line **is** the pivot — full mechanics (and `-L`/`-R`/`-D` forwarding) live in [SSH - Tunneling and Jump Hosts](../02%20-%20Recon%20and%20Network%20Tools/SSH%20-%20Tunneling%20and%20Jump%20Hosts.md).
...
## Related
- [SSH - Tunneling and Jump Hosts](../02%20-%20Recon%20and%20Network%20Tools/SSH%20-%20Tunneling%20and%20Jump%20Hosts.md)
- [Chisel](../02%20-%20Recon%20and%20Network%20Tools/Chisel.md)
- [iptables](../03%20-%20Linux%20Skills/iptables.md)
- [WireGuard](../02%20-%20Recon%20and%20Network%20Tools/WireGuard.md)
- [OpenVPN](../02%20-%20Recon%20and%20Network%20Tools/OpenVPN.md)
